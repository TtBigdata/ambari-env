#!/usr/bin/env bash
#
# Rocky Linux 8.10 bare-metal initializer for the Ambari build environment.
#
...
#
# Usage:
#   sudo bash scripts/system/init-no-docker/init_rocky8_no_docker.sh
#
# Useful overrides:
#   CONFIGURE_DNF_MIRROR=0      keep only the server's existing dnf/yum sources
#   ROCKY_MIRROR=https://...    public Rocky mirror used when CONFIGURE_DNF_MIRROR=1
#   EPEL_MIRROR=https://...     public EPEL mirror used when CONFIGURE_DNF_MIRROR=1
...
CONFIGURE_DNF_MIRROR="${CONFIGURE_DNF_MIRROR:-1}"
ROCKY_MIRROR="${ROCKY_MIRROR:-https://mirrors.aliyun.com/rockylinux}"
EPEL_MIRROR="${EPEL_MIRROR:-https://mirrors.aliyun.com/epel}"
DNF_DISABLED_REPOS="${DNF_DISABLED_REPOS:-docker-ce-*}"
...
require_rocky8() {
  ...
  case "${ID:-}_${VERSION_ID:-}" in
    rocky_8* | rockylinux_8* )
      ;;
    *)
      die "this script supports Rocky Linux 8.x only, current system is ${ID:-unknown} ${VERSION_ID:-unknown}"
      ;;
  esac
}
...
configure_dnf_mirror() {
  if [ "$CONFIGURE_DNF_MIRROR" != "1" ]; then
    log "DNF mirror rewrite skipped; using existing server sources."
    return
  fi

  local repo_file="/etc/yum.repos.d/ambari-env-rocky8.repo"
  local rocky="${ROCKY_MIRROR%/}"
  local epel="${EPEL_MIRROR%/}"

  log "Writing public Rocky/EPEL repositories: ${repo_file}"
  cat >"$repo_file" <<EOF
[ambari-rocky-baseos]
name=Rocky Linux \$releasever - BaseOS
baseurl=${rocky}/\$releasever/BaseOS/\$basearch/os/
enabled=1
gpgcheck=0

[ambari-rocky-appstream]
name=Rocky Linux \$releasever - AppStream
baseurl=${rocky}/\$releasever/AppStream/\$basearch/os/
enabled=1
gpgcheck=0

[ambari-rocky-extras]
name=Rocky Linux \$releasever - Extras
baseurl=${rocky}/\$releasever/extras/\$basearch/os/
enabled=1
gpgcheck=0

[ambari-rocky-powertools]
name=Rocky Linux \$releasever - PowerTools
baseurl=${rocky}/\$releasever/PowerTools/\$basearch/os/
enabled=1
gpgcheck=0

[ambari-epel]
name=EPEL for Rocky Linux 8 - \$basearch
...
set_dnf_repo_args() {
  ...
  if [ "$CONFIGURE_DNF_MIRROR" = "1" ]; then
    DNF_REPO_ARGS=(
      --disablerepo="*"
      --enablerepo=ambari-rocky-baseos
      --enablerepo=ambari-rocky-appstream
      --enablerepo=ambari-rocky-extras
      --enablerepo=ambari-rocky-powertools
      --enablerepo=ambari-epel
    )
  elif [ -n "$DNF_DISABLED_REPOS" ]; then
    DNF_REPO_ARGS=(--disablerepo="$DNF_DISABLED_REPOS")
  fi
}
...
install_mariadb_devel_headers() {
  ...
  configure_mariadb_repo_hotfixes
  if dnf -q --setopt=mariadb.module_hotfixes=true list MariaDB-devel >/dev/null 2>&1; then
    dnf -y --setopt=mariadb.module_hotfixes=true install MariaDB-devel
    return
  fi

  dnf_with_repos -y install mariadb-devel || dnf_with_repos -y install mariadb-connector-c-devel
}

install_packages() {
  log "Installing Rocky build dependencies"
  disable_problem_repos
  set_dnf_repo_args
  configure_mariadb_repo_hotfixes
  dnf clean all || true
  dnf_with_repos makecache -y || true

  dnf_with_repos -y install dnf-plugins-core yum-utils || true
  dnf_with_repos config-manager --set-enabled powertools >/dev/null 2>&1 || true
  dnf_with_repos config-manager --set-disabled docker-ce-stable docker-ce-test docker-ce-nightly >/dev/null 2>&1 || true

  dnf_with_repos -y groupinstall "Development Tools" || true
  ...
  dnf_with_repos -y install "${core_packages[@]}"
  install_mariadb_devel_headers

  local package
  for package in "${optional_packages[@]}"; do
    dnf_with_repos -y install "$package" || log "Optional package unavailable, skipped: ${package}"
  done
}
...
repair_java_cacerts() {
  log "Repairing Java CA trust stores"
  local target="/etc/pki/java/cacerts"
  local source=""
  local secdir
  ...
  elif [ -s "${JDK8_HOME}/jre/lib/security/cacerts" ]; then
    source="${JDK8_HOME}/jre/lib/security/cacerts"
  elif [ -s "${JDK8_HOME}/lib/security/cacerts" ]; then
    source="${JDK8_HOME}/lib/security/cacerts"
  fi
  ...
  if [ ! -s "$target" ]; then
    log "No Java cacerts source found; Maven HTTPS may fail if a system OpenJDK trust store is broken."
    return
  fi

  for secdir in \
    /usr/lib/jvm/java-*-openjdk*/jre/lib/security \
    /usr/lib/jvm/java-*-openjdk*/lib/security \
    /etc/java/java-*-openjdk/*/lib/security; do
    [ -d "$secdir" ] || continue
    ln -sfn "$target" "${secdir}/cacerts"
  done

  if command -v keytool >/dev/null 2>&1; then
    keytool -list -keystore "$target" -storepass changeit >/dev/null 2>&1 \
      || log "Java cacerts exists but keytool could not list it; leaving file in place: ${target}"
  fi
}
...
# Managed by ambari-env init_rocky8_no_docker.sh
...
export PATH="\$JAVA_HOME/bin:\$CMAKE_HOME/bin:\$MAVEN_HOME/bin:\$GRADLE_HOME/bin:\$ANT_HOME/bin:\$PHANTOMJS_HOME/bin:\$R_PREFIX/bin:\$PYTHON37_HOME/bin:\$HADOOP_PY37_VENV/bin:\$CONDA_HOME/bin:\$PATH"
...
verify_installation() {
  ...
  export JAVA_HOME="$JDK8_HOME"
  export MAVEN_HOME="$MAVEN_ACTIVE_HOME"
  export PATH="$JAVA_HOME/bin:$CMAKE_HOME/bin:$MAVEN_HOME/bin:$GRADLE_HOME/bin:$ANT_HOME/bin:$PHANTOMJS_HOME/bin:$R_HOME/bin:$PYTHON_HOME/bin:$PY37_VENV/bin:$PATH"
...
main() {
  ...
  verify_installation
  log "Ambari Rocky 8 bare-metal initialization finished."
}
